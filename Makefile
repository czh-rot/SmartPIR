...
ifeq ($(HOST_ARCH), x86)
	LDFLAGS += -L$(XILINX_HLS)/lnx64/tools/fpo_v7_0 -Wl,--as-needed 
endif
...
HOST_ARGS := --xclbin $(BUILD_DIR)/cryptonets.xclbin --scale $(TB) --seed $(SEED)

# ##################### Kernel compiler global settings ##########################
VPP_FLAGS += -t $(TARGET) --platform $(XPLATFORM) --save-temps --optimize 2
VPP_FLAGS += --hls.jobs 8
VPP_LDFLAGS += --vivado.synth.jobs 4 --vivado.impl.jobs 4

VPP_FLAGS += -I$(CUR_DIR)/kernel

cryptonets_VPP_FLAGS +=  -D KERNEL_NAME=cryptonets
cryptonets_VPP_FLAGS += --hls.clock 300000000:cryptonets
VPP_LDFLAGS_cryptonets += --kernel_frequency 300
...
BINARY_CONTAINERS += $(BUILD_DIR)/cryptonets.xclbin
BINARY_CONTAINER_cryptonets_OBJS += $(TEMP_DIR)/cryptonets.xo
...
$(TEMP_DIR)/cryptonets.xo: $(CUR_DIR)/kernel/top.cpp $(CUR_DIR)/kernel/ntt.cpp $(CUR_DIR)/kernel/top.h $(CUR_DIR)/kernel/ntt.h $(CUR_DIR)/kernel/add.h  $(CUR_DIR)/kernel/components.cpp $(CUR_DIR)/kernel/define.h $(CUR_DIR)/kernel/intt.cpp  $(CUR_DIR)/kernel/multiply.cpp $(CUR_DIR)/kernel/rescale.cpp $(CUR_DIR)/kernel/rotation.cpp $(CUR_DIR)/kernel/switchkey.cpp 
	$(ECHO) "Compiling Kernel: cryptonets"
	mkdir -p $(TEMP_DIR)
	$(VPP) -c $(cryptonets_VPP_FLAGS) $(VPP_FLAGS) -k cryptonets -I'$(<D)' --temp_dir $(TEMP_DIR) --report_dir $(TEMP_REPORT_DIR) -o'$@' $^


$(BUILD_DIR)/cryptonets.xclbin: $(BINARY_CONTAINER_cryptonets_OBJS)
	mkdir -p $(BUILD_DIR)
	$(VPP) -l $(VPP_FLAGS) --temp_dir $(TEMP_DIR) --report_dir $(BUILD_REPORT_DIR)/cryptonets $(VPP_LDFLAGS) $(VPP_LDFLAGS_cryptonets) -o '$@' $(+)
